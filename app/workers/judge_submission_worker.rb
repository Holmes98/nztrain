...
class JudgeSubmissionWorker < ApplicationWorker
  ...
  def run_interactive(test_case, run_command, eval_command, resource_limits)
    ...
    interactive_boxes.each_with_index do |box, index|
      ...
      grader_to_user = IO.pipe
      user_to_grader = IO.pipe
      grader_pipes << [user_to_grader[0], grader_to_user[1]]

      run_opts = resource_limits.merge(:meta => metafiles.last.path, :err => boxfiles.last, :in => grader_to_user[0], :out => user_to_grader[1], :stderr => logfiles.last)
      ...
      grader_to_user[0].close
      user_to_grader[1].close
    end

    expected = conditioned_output(test_case.output)
    FileUtils.copy(File.expand_path(EvalFileName, tmpdir), box.expand_path(EvalFileName))
    grader_resource_limits = { :mem => 524288, :time => num_processes * (time_limit + extra_time + 1) + 15, :wall_time => num_processes * wall_time + 30 }

    eval_output = nil
    eval_result = {}
    str_to_pipe(expected) do |output_stream|
      run_opts = grader_resource_limits.merge(:processes => true, 4 => output_stream, :stdin_data => test_case.input, :output_limit => OutputBaseLimit + test_case.output.bytesize*4, :clean_utf8 => true, :inherit_fds => true)
      grader_pipes.each_with_index do |pipes, index|
