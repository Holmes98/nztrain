...
class JudgeSubmissionWorker < ApplicationWorker
  ...
  def run_interactive(test_case, run_command, eval_command, resource_limits)
    ...
    interactive_boxes.each_with_index do |box, index|
      ...
      judge_to_user = IO.pipe
      user_to_judge = IO.pipe
      judge_pipes << [user_to_judge[0], judge_to_user[1]]

      run_opts = resource_limits.merge(:meta => metafiles.last.path, :err => boxfiles.last, :in => judge_to_user[0], :out => user_to_judge[1], :stderr => logfiles.last)
      ...
      judge_to_user[0].close
      user_to_judge[1].close
    end

    expected = conditioned_output(test_case.output)
    FileUtils.copy(File.expand_path(EvalFileName, tmpdir), box.expand_path(EvalFileName))
    judge_resource_limits = { :mem => 524288, :time => num_processes * (time_limit + extra_time + 1) + 15, :wall_time => num_processes * wall_time + 30 }

    eval_output = nil
    eval_result = {}
    str_to_pipe(expected) do |output_stream|
      run_opts = judge_resource_limits.merge(:processes => true, 3 => output_stream, :stdin_data => test_case.input, :output_limit => OutputBaseLimit + test_case.output.bytesize*4, :clean_utf8 => true, :inherit_fds => true)
      judge_pipes.each_with_index do |pipes, index|
        run_opts.merge!(index * 2 + 4 => pipes[0], index * 2 + 5 => pipes[1])
      end
